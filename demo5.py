...
def main():
    ...
    circle_radius = st.sidebar.selectbox("추천 지점 주변 원의 반경 (미터)", options=[0, 100, 200], index=1)
    ...
    col1, col2 = st.columns([1, 4])  # 비율을 조정하여 공간 분배

    with col1:
        # 로고 이미지 삽입 (로컬 이미지 파일 사용)
        logo_path = "logo1.png"  # 로고 이미지 파일 경로
    ...
    if st.sidebar.button("예측 실행"):
        ...
        st.write(f"**마지막 GPS 위치:** 위도 = {last_lat}, 경도 = {last_lon}")
